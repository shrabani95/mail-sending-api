    ...
    async  Get_AllRequests()
        {
            ...
            if(data1.status===1)
            {
              ...
              this.setState({
                ...
                reminderCount:data.reminderDetails!==null && data.reminderDetails.length > 0 ? data.reminderDetails[0].reminder:0,
                noOfContest:data.contest.noOfContest,
                newRequest:data.newRequest!==null && data.newRequest.length > 0 ? data.newRequest[0].pendingRequest : 0,
               });
                                ...
                                <div className="col-md-8">
                                    ...
                                    <div className="header-button mobileTopbar">
                                        ...
                                        {/* <NavLink  to={{pathname: '/notify',state: {allRequest : this.state.allRequest}}} ><NotificationsOutlinedIcon/>{this.state.newRequest > 0 ? <span className=""> {this.state.newRequest}</span> : null}</NavLink>    */}
                                       {
                                         ...
                                         <button  className="btun Mobile-notification" onClick={this.GotoViewNotify} ><NotificationsOutlinedIcon /><span className=""> { (this.state.newRequest + this.state.reminderCount)  }</span></button>   
                                          :
                                          this.state.reminderCount > 0 && this.state.newRequest===0 ?
                                          <button  className="btun Mobile-notification" onClick={this.GotoViewNotify} ><NotificationsOutlinedIcon /> <span className=""> { this.state.reminderCount  }</span></button>   
                                         :
                                          this.state.reminderCount === 0 && this.state.newRequest > 0 ?
                                          <button  className="btun Mobile-notification" onClick={this.GotoViewNotify} ><NotificationsOutlinedIcon /><span className=""> { this.state.newRequest  }</span> </button>   
                                          :
